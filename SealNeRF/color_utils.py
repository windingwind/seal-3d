# copied from https://github.com/limacv/RGB_HSV_HSL/blob/master/color_torch.py
# accept [B,3,H,W]/[B,3,N] input
...
def rgb2hsl_torch(rgb: torch.Tensor) -> torch.Tensor:
    ...
    delta = cmax - cmin
    hsl_h = torch.empty_like(rgb[:, 0:1, :]) if rgb.ndim == 3 else torch.empty_like(rgb[:, 0:1, :, :])
    cmax_idx[delta == 0] = 3
...
def rgb2hsv_torch(rgb: torch.Tensor) -> torch.Tensor:
    ...
    cmin = torch.min(rgb, dim=1, keepdim=True)[0]
    delta = cmax - cmin
    hsv_h = torch.empty_like(rgb[:, 0:1, :]) if rgb.ndim == 3 else torch.empty_like(rgb[:, 0:1, :, :])
    cmax_idx[delta == 0] = 3
...
def hsv2rgb_torch(hsv: torch.Tensor) -> torch.Tensor:
    ...
    idx = (idx % 6).expand(-1, 3, -1) if hsv.ndim == 3 else (idx % 6).expand(-1, 3, -1, -1)
...
def hsl2rgb_torch(hsl: torch.Tensor) -> torch.Tensor:
    ...
    idx = (idx % 6).expand(-1, 3, -1) if hsl.ndim == 3 else (idx % 6).expand(-1, 3, -1, -1)

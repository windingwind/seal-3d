...
# custom version of NeRFNetwork
class NeRFNetwork(SealNeRFRenderer):
    def __init__(self,
                 resolution=[128] * 3,
                 sigma_rank=[16] * 3,
                 color_rank=[48] * 3,
        ...
        self.sigma_mat, self.sigma_vec = self.init_one_svd(
            self.sigma_rank, self.resolution)
        self.color_mat, self.color_vec = self.init_one_svd(
        ...
        self.encoder, enc_dim = get_encoder(
            'frequency', input_dim=color_feat_dim, multires=2)
        self.encoder_dir, enc_dim_dir = get_encoder(
            'frequency', input_dim=3, multires=2)
        ...
        if self.bg_radius > 0:
            ...
            self.bg_mat = nn.Parameter(
                0.1 * torch.randn((1, bg_rank, bg_resolution[0], bg_resolution[1])))  # [1, R, H, W]
    ...
    def init_one_svd(self, n_component, resolution, scale=0.1):
        ...
        for i in range(len(self.vec_ids)):
            ...
            mat.append(nn.Parameter(
                scale * torch.randn((1, n_component[i], resolution[mat_id_1], resolution[mat_id_0]))))  # [1, R, H, W]
            # [1, R, D, 1] (fake 2d to use grid_sample)
            vec.append(nn.Parameter(
    ...
    def get_sigma_feat(self, x):
        ...
        # plane + line basis
        mat_coord = torch.stack((x[..., self.mat_ids[0]], x[..., self.mat_ids[1]],
                                x[..., self.mat_ids[2]])).view(3, -1, 1, 2)  # [3, N, 1, 2]
        vec_coord = torch.stack(
            (x[..., self.vec_ids[0]], x[..., self.vec_ids[1]], x[..., self.vec_ids[2]]))
        vec_coord = torch.stack((torch.zeros_like(
            vec_coord), vec_coord), dim=-1).view(3, -1, 1, 2)  # [3, N, 1, 2], fake 2d coord

        sigma_feat = torch.zeros([N,], device=x.device)

        for i in range(len(self.sigma_mat)):
            mat_feat = F.grid_sample(self.sigma_mat[i], mat_coord[[
                                     i]], align_corners=True).view(-1, N)  # [1, R, N, 1] --> [R, N]
            vec_feat = F.grid_sample(
                self.sigma_vec[i], vec_coord[[i]], align_corners=True).view(-1, N)  # [R, N]
    ...
    def get_color_feat(self, x):
        ...
        for i in range(len(self.color_mat)):
            mat_feat.append(F.grid_sample(self.color_mat[i], mat_coord[[
                            i]], align_corners=True).view(-1, N))  # [1, R, N, 1] --> [R, N]
            vec_feat.append(F.grid_sample(self.color_vec[i], vec_coord[[
                            i]], align_corners=True).view(-1, N))  # [R, N]

        mat_feat = torch.cat(mat_feat, dim=0)  # [3 * R, N]
        vec_feat = torch.cat(vec_feat, dim=0)  # [3 * R, N]

        # [N, 3R] --> [N, color_feat_dim]
        color_feat = self.basis_mat((mat_feat * vec_feat).T)
    ...
    def forward(self, x, d):
        ...
        x = 2 * (x - self.aabb_train[:3]) / \
            (self.aabb_train[3:] - self.aabb_train[:3]) - 1
    ...
    def background(self, x, d):
        ...
        h = F.grid_sample(self.bg_mat, x.view(
            1, N, 1, 2), align_corners=True).view(-1, N).T.contiguous()  # [R, N] --> [N, R]
    ...
    def density_loss(self):
        ...
        for i in range(len(self.sigma_mat)):
            loss = loss + \
                torch.mean(
                    torch.abs(self.sigma_mat[i])) + torch.mean(torch.abs(self.sigma_vec[i]))
        return loss
    ...
    def upsample_params(self, mat, vec, resolution):
        ...
        for i in range(len(self.vec_ids)):
            vec_id = self.vec_ids[i]
            mat_id_0, mat_id_1 = self.mat_ids[i]
            mat[i] = nn.Parameter(F.interpolate(mat[i].data, size=(
                resolution[mat_id_1], resolution[mat_id_0]), mode='bilinear', align_corners=True))
            vec[i] = nn.Parameter(F.interpolate(vec[i].data, size=(
                resolution[vec_id], 1), mode='bilinear', align_corners=True))
    ...
    def shrink_model(self):
        ...
        valid_pos = raymarching.morton3D_invert(torch.nonzero(
            valid_grid))  # [Nz] --> [Nz, 3], in [0, H - 1]
        # plot_pointcloud(valid_pos.detach().cpu().numpy()) # lots of noisy outliers in hashnerf...
        valid_pos = (2 * valid_pos / (self.grid_size - 1) - 1) * \
            (self.bound - half_grid_size)  # [Nz, 3], in [-b+hgs, b-hgs]
        ...
        for i in range(len(self.vec_ids)):
            ...
            self.sigma_mat[i] = nn.Parameter(
                self.sigma_mat[i].data[..., tl[mat_id_1]:br[mat_id_1], tl[mat_id_0]:br[mat_id_0]])
            self.color_mat[i] = nn.Parameter(
                self.color_mat[i].data[..., tl[mat_id_1]:br[mat_id_1], tl[mat_id_0]:br[mat_id_0]])
...
class SealNeRFTeacherNetwork(SealNeRFTeacherRenderer, NeRFNetwork):
    def __init__(self,
                 resolution=[128] * 3,
                 sigma_rank=[16] * 3,
                 color_rank=[48] * 3,
        ...
        super().__init__(bound=bound, resolution=resolution, sigma_rank=sigma_rank, color_rank=color_rank, bg_resolution=bg_resolution, bg_rank=bg_rank, color_feat_dim=color_feat_dim, num_layers=num_layers, hidden_dim=hidden_dim,
                         num_layers_bg=num_layers_bg, hidden_dim_bg=hidden_dim_bg, **kwargs)


class SelaNeRFStudentNetwork(SealNeRFStudentRenderder, NeRFNetwork):
    def __init__(self,
                 resolution=[128] * 3,
                 sigma_rank=[16] * 3,
                 color_rank=[48] * 3,

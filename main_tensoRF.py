...
if __name__ == '__main__':
    ...
    if opt.test:
        ...
        trainer = Trainer('tensoRF', opt, model, device=device, workspace=opt.workspace, criterion=criterion, fp16=opt.fp16, metrics=[PSNRMeter()], use_checkpoint=opt.ckpt)
        ...
        if opt.gui:
            ...
            test_loader = NeRFDataset(opt, device=device, type=opt.test_type).dataloader()
        ...
        trainer = Trainer('tensoRF', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=None, fp16=opt.fp16, lr_scheduler=scheduler, scheduler_update_every_step=True, metrics=[PSNRMeter()], use_checkpoint=opt.ckpt, eval_interval=50)
